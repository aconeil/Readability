...
def getlevel():
    ...
    iso = flask.request.args.get('iso')
    return flask.redirect('/readability'+flask.url_for('lang', iso=iso, level=level))
...
def load_data(iso):
    ...
    if value == None:
        ...
        if len(ranked_data) < 101 or len(sparse_data) > 100:
            list = random.sample(range(1000), 200)
            for i in range(0, 200, 2):
                best_hundred.append([0, list[i], list[i+1]])
            con.close()
